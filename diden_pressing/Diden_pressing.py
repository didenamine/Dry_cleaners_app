...
Prename_client_Entry=Entry(width=20,font=('arial',15))
...
def show_all_records() :
    database_connect=sqlite3.connect('Pressing_base_donne.db')
    database_cursor=database_connect.cursor()
    database_cursor.execute("SELECT name_client,prename_client,phone_client,date_commande,hour_commande,sum(clothe_total_price),Paid_y_n from COMMANDE where Paid_y_n='OUI' GROUP BY phone_client,name_client,prename_client order by date_commande ASC,hour_commande ASC")
...
def show_all_unpaid() :
    database_connect=sqlite3.connect('Pressing_base_donne.db')
    database_cursor=database_connect.cursor()
    database_cursor.execute("SELECT name_client,prename_client,phone_client,date_commande,hour_commande,sum(clothe_total_price),Paid_y_n from COMMANDE where Paid_y_n='NON' GROUP BY phone_client,name_client,prename_client order by date_commande ASC,hour_commande ASC")
